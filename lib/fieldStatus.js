...
async function setFieldStatus(isClosed, reason = "") {
  ...
  if (!res.ok) {
    ...
    const tokenPreview = vercelToken ? `${vercelToken.slice(0, 6)}...` : "MISSING";
    const idPreview = edgeConfigId || "MISSING";
    throw new Error(`Edge Config write failed (${res.status}): ${body} [token=${tokenPreview}, id=${idPreview}]`);
  }

  return status;
}

module.exports = { getFieldStatus, setFieldStatus };
