...
function generateLocationApiUrl(lat, lng) {
  ...
  const dataSets = "forecastNextHour";
...
async function getLocationNext(lat, lng) {
  ...
  try {
    ...
    console.error("Error fetching location next hour:", error);
    throw error;
  }
}
...
export default async function handler(req, res) {
  try {
    ...
    const weatherData = await getLocationNext(latitude, longitude);
    
    res.json(weatherData);
  } catch (error) {
    console.error("API Error:", error);
    res.status(500).json({ 
      error: "Failed to fetch next hour data for location" 
    });
  }
}