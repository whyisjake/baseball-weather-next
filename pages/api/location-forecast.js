...
async function getLocationForecast(lat, lng) {
...
export default async function handler(req, res) {
  try {
    ...
    const weatherData = await getLocationForecast(latitude, longitude);
    
    res.json(weatherData);
  } catch (error) {
    console.error("API Error:", error);
    res.status(500).json({ 
      error: "Failed to fetch forecast data for location" 
    });
  }
}